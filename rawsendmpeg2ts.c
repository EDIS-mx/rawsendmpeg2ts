...
static uint64_t derive_rate(FILE *f) {
    uint8_t pkt[TS];
...
static void pump(FILE *f, int sock, uint64_t R) {
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr, "usage: %s <file.ts> <ip:port>   (IPv4 only)\n", argv[0]);
        return 2;
    }
    ...
    uint64_t R = derive_rate(f);
    fprintf(stderr, "derived rate: %llu bit/s (%.3f Mb/s)\n",
            (unsigned long long)R, R / 1e6);

    if (fseeko(f, 0, SEEK_SET) != 0) die_errno("fseeko");   /* rewind before the pump */
    pump(f, sock, R);
